...
export async function verify(
  ...
  // All checks passed — populate `fields` based on which variant is present.
  // validateStructure guaranteed exactly one of (invoice, marketClaim).
  if (doc.invoice) {
    return {
      ok: true,
      fields: {
  ...
  // doc.marketClaim is guaranteed by validateStructure.
