...
import { runLendingIndexer } from "../server/lending/indexer.js";

/**
 * GET/POST /api/lending-index-cron
 *
 * Protected automation endpoint that runs one indexer pass: walks new
 * blocks since `lending_indexer_state.last_scanned_block`, mirrors events,
 * refreshes affected positions, and writes one pool snapshot.
 *
 * Auth: Bearer CRON_SECRET or x-admin-key: ADMIN_API_KEY. Same pattern as
 * markets-operator-cron.
 */
export default async function handler(
  ...
  try {
    ...
    const result = await runLendingIndexer();
