...
export async function generateInvoicePdf(input: InvoiceInput): Promise<Uint8Array> {
  ...
  if (input.pspDigest) {
    ...
    const verifyCommand = input.pspUid
      ? `Verify: curl -s "${input.pspVerifierUrl || "https://app.disburse.online"}/api/psp?uid=${input.pspUid}" | npx @disburse/psp-verify --stdin`
      : "Verify: npx @disburse/psp-verify disburse-psp-....json";
    page.drawText(verifyCommand, {
      x: MARGIN,
      y: 60,
      size: 6.5,
      font: regular,
